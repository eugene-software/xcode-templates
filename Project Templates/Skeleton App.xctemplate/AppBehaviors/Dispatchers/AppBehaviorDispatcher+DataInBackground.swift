...
public extension AppBehaviorDispatcher {
    ...
    func application(_ application: UIApplication, performFetchWithCompletionHandler completionHandler: @escaping (UIBackgroundFetchResult) -> Swift.Void) {
        ...
        apply({ (behavior, completionHandler) -> Void? in
            behavior.application?(application, performFetchWithCompletionHandler: completionHandler)
...
//            behavior.application?(application, handleEventsForBackgroundURLSession: identifier, completionHandler: completionHandler)
//        }
        apply({ (behavior, handler) -> Void? in
            behavior.application?(application, handleEventsForBackgroundURLSession: identifier) {
                handler(())
            }
